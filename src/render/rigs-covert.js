...
const dagger = (ctx, x, y, a, col, o = {}) => part(ctx, (c) => {
  const to = along(x, y, a), len = o.len ?? 5.2;
  line(c, ...to(-1.3), ...to(0.5), 1.0, "#3a2418");
  dab(c, ...to(-1.5).map((v) => v - 0.45), 0.9, 0.9, o.trim || "#c4c8d0");                // the pommel
  line(c, ...to(0.7, -1.2), ...to(0.7, 1.2), 0.7, o.trim || "#c4c8d0");                 // the guard
  const blade = [[...to(0.9, -0.62), 1], [...to(len * 0.55, -0.72)], [...to(len, 0), 1], [...to(len * 0.55, 0.72)], [...to(0.9, 0.62), 1]];
  path(c, blade); c.fillStyle = darken(col, 0.18); c.fill();
  path(c, [[...to(0.9, -0.62), 1], [...to(len * 0.55, -0.72)], [...to(len, 0), 1], [...to(0.9, 0), 1]]);
  c.fillStyle = lighten(col, 0.4); c.fill();
  line(c, ...to(1.2, 0), ...to(len * 0.8, 0), 0.3, lighten(col, 0.7));                    // the bevel's glint
  if (o.venom) { line(c, ...to(len * 0.35, 0.62), ...to(len * 0.92, 0.18), 0.45, o.venom); dab(c, ...to(len * 0.5, 0.95), 0.5, 0.6, o.venom); }
});
...
const hoodHead = (ctx, x, y, a, p) => inFrame(ctx, x, y, a, (c0) => {
  ...
  blob(c0, H, hood, {
    hi: 0.38, lo: 0.5, then: (c) => {
      ...
      dab(c, 1.45, -0.55, 0.85, 0.55, p.eyes || "#f0e0b0");                    // the eye's glint
      dab(c, 1.45, -0.55, 0.4, 0.28, lighten(p.eyes || "#f0e0b0", 0.6));
      ...
      line(c, -1.6, -3.6, 2.6, -2.7, 0.5, lighten(hood, 0.45));
      line(c, 2.4, -2.9, 3.2, -1.7, 0.45, lighten(hood, 0.35));
...
const blade = (ctx, p) => {
  ...
  inFrame(ctx, R.hip[0], R.hip[1], st.lean, (c) => {
    ...
    blob(c, pts, cloak, {
      hi: 0.34, lo: 0.4, then: (cc) => {
        line(cc, -1.7, -5.0, -3.0 - fl, 3.8 - lift, 0.5, darken(cloak, 0.4));
        line(cc, -1.0, -3.4, -2.2 - fl * 0.5, 4.2 - lift * 0.5, 0.45, darken(cloak, 0.35));
        line(cc, -2.4, -6.6, -2.9 - fl * 0.2, -2.6, 0.5, lighten(cloak, 0.35));
        // the lining shows at the hem
        line(cc, -4.2 - fl * 1.3, 4.5 - lift, -1.2 - fl * 0.3, 4.6 - lift * 0.4, 0.5, mix(cloak, sash, 0.5));
      },
    });
  });
  ...
  inFrame(ctx, R.hip[0], R.hip[1], st.lean, (c) => {
    ...
    blob(c, [[-0.6, -0.9], [0.9, -0.9], [1.0, 0.9, 1], [-0.5, 1.0, 1]], darken(lea, 0.1), { hi: 0.35, then: (cc) => { dab(cc, -0.8, -0.9, 2.2, 0.5, darken(lea, 0.4)); dab(cc, 0.1, -0.5, 0.4, 0.4, trim); } });
  ...
  dagger(ctx, hn[0], hn[1], H.an, steel, { len: st.fight ? 5.4 : 4.8, trim, venom: p.venom });
